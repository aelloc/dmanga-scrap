...
async function retrieveChapterPages (chapter: Chapter): Promise<Page[]> {
  function selectPages (): Page[] {
    ...
    const pages = Array.from(elements).map(elem => {
...
async function retrieveChapters (manga: Manga): Promise<Chapter[]> {
  function selectChapters (): string[] {
    ...
    const chapters = Array.from(elements).reverse().map(elem => elem.textContent)
  ...
  const chapters = names.map(name => {
    const pathname = resolve(manga.url.pathname, name)
